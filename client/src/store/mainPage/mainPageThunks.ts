  ...
  async (_, thunkAPI) => {
    try {
      ...
      return response.data.data.map((room: any) => ({
        ...
        isCreator: false, // Will be set by the component based on userId
      }));
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } catch (err: any) {
...
export const createRoom = createAsyncThunk<
  ...
  { rejectValue: string }
>('mainPage/createRoom', async ({ roomName, password }, thunkAPI) => {
  try {
    const response = await api.post('/api/room/new', {
      phase_id: 1,
      current_topic_id: 1,
...
>('mainPage/updateRoom', async ({ id, room_name }, thunkAPI) => {
  try {
    const response = await api.put(`/api/room/change/${id}`, { room_name });
    return {
      id: response.data.data.id,
      title: response.data.data.room_name,
      room_creator: response.data.data.room_creator,
      room_code: response.data.data.room_code,
      isCreator: false, // Will be set by the component based on userId
    };
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  } catch (err: any) {
