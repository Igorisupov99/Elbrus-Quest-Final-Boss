...
class QuestionController {
  ...
  async getQuestion(req, res) {
    try {
      ...
      const randomQuestion = await Question.findOne({
        ...
        order: [['id', 'ASC']], //  определяем порядок вывода данных
        offset: randomOffset,
        limit: 1
      });
      ...
      return res.json({ question_text: randomQuestion.question_text, question_id: randomQuestion.id });
    } catch (error) {
  ...
  async answerCheck(req, res) {
    try {
        ...
        // Сравниваем ответ клиента с правильным ответом
        // Можно сделать сравнение без учёта регистра и пробелов
